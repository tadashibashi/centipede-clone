...
if (global.game_mode == gameMode.mush) { //Mode 3
   ...
   if (global.demo == false) {gmwPostEvent(global.EV_bgBloop_stop,sound_controller.id);} //If demo stop sound
...
if (global.game_mode = gameMode.dead) { //Mode4
    ...
    if (healing == 1) {
       ...
       if (lives >= 2) && (global.demo = false) {
                 ...
                 gmwPostEvent(global.EV_bgBloop_stop,sound_controller.id);//stops background blooping sound
